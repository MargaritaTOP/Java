...
public class Array28 {
    public static int findMinEvenIndexedElement(int[] array) {
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив должен быть не null и содержать как минимум 1 элемент");
        }
        // Используем Stream для поиска минимального элемента среди элементов с четными номерами (индексы 1, 3, 5, ...)
        return IntStream.range(1, array.length)
                .filter(i -> i % 2 == 1) // Четные номера соответствуют нечетным индексам (1-based: A2 -> index 1)
                .map(i -> array[i])
                .min()
                .orElse(0); // Возвращаем 0, если нет элементов с четными номерами
    }
    public static void main(String[] args) {
        ...
        // Поиск минимального элемента среди элементов с четными номерами и вывод результата
        try {
            int result = findMinEvenIndexedElement(array);
            System.out.println("Минимальный элемент среди элементов с четными номерами (A2, A4, ...): " + result);
