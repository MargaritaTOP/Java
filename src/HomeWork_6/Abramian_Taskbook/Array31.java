...
import java.util.stream.IntStream;
 ...
 /* УСЛОВИЕ ЗАДАЧИ
 Дан массив размера N. Найти номера тех элементов массива, которые больше своего левого соседа, и количество таких элементов.
 Найденные номера выводить в порядке их убывания.
 */

public class Array31 {
    public static int[] findGreaterThanLeftNeighborIndices(int[] array) {
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив должен быть не null и содержать как минимум 1 элемент");
        }
        // Используем Stream для поиска индексов элементов, которые больше своего левого соседа
        return IntStream.range(1, array.length) // Начинаем с индекса 1, так как у первого элемента нет левого соседа
                .filter(i -> array[i] > array[i - 1]) // Элемент больше предыдущего
                .map(i -> i + 1) // Преобразуем индекс в номер
                .boxed() // Преобразуем в Stream<Integer> для сортировки по убыванию
    ...
    public static void main(String[] args) {
        ...
        // Поиск номеров элементов, больших левого соседа, и вывод результата
        try {
            int[] result = findGreaterThanLeftNeighborIndices(array);
            System.out.print("Номера элементов, больших своего левого соседа: ");
            if (result.length == 0) {
                System.out.println("нет");
            } else {
                System.out.println(Arrays.toString(result));
            }
            System.out.println("Количество таких элементов: " + result.length);
