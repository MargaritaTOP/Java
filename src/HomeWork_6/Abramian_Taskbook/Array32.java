...
public class Array32 {
    public static int findFirstLocalMinimumIndex(int[] array) {
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив должен быть не null и содержать как минимум 1 элемент");
        }
        // Для массива из одного элемента он считается локальным минимумом
        if (array.length == 1) {
            return 1;
        }
        // Используем Stream для поиска первого локального минимума
        return IntStream.range(0, array.length)
                .filter(i -> {
                    if (i == 0) {
                        return array[0] < array[1]; // Первый элемент
                    } else if (i == array.length - 1) {
                        return array[i] < array[i - 1]; // Последний элемент
                    } else {
                        return array[i] < array[i - 1] && array[i] < array[i + 1]; // Средние элементы
                    }
                })
                .map(i -> i + 1) // Преобразуем индекс в номер (1-based)
                .findFirst()
                .orElse(0); // Возвращаем 0, если локальный минимум не найден
    }
    public static void main(String[] args) {
        ...
        // Поиск номера первого локального минимума и вывод результата
        try {
            int result = findFirstLocalMinimumIndex(array);
            System.out.println("Номер первого локального минимума: " + result);
