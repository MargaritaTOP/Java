...
public class Array48 {
    public static class Result {
        long count; // Максимальное количество одинаковых элементов
        int value;  // Значение элемента с максимальной частотой

        Result(long count, int value) {
            this.count = count;
            this.value = value;
        }
    }
    public static Result findMaxDuplicateCount(int[] array) {
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив должен быть не null и содержать как минимум 1 элемент");
        }
        // Используем Stream для подсчета частоты элементов
        ...
        Map.Entry<Integer, Long> maxEntry = frequencyMap.entrySet().stream()
                ...
                .orElseThrow(() -> new IllegalStateException("Не удалось найти максимальную частоту"));
        return new Result(maxEntry.getValue(), maxEntry.getKey());
    }
    public static void main(String[] args) {
        ...
        // Подсчет максимального количества одинаковых элементов и вывод результата
        try {
            Result result = findMaxDuplicateCount(array);
            System.out.println("Максимальное количество одинаковых элементов: " + result.count);
            System.out.println("Элемент, встречающийся " + result.count + " раз: " + result.value);
