...
import java.util.stream.IntStream;
 ...
 /* УСЛОВИЕ ЗАДАЧИ
 Дан массив размера N. Найти номера тех элементов массива, которые больше своего правого соседа, и количество таких элементов.
 Найденные номера выводить в порядке их возрастания.
 */

public class Array30 {
    public static int[] findGreaterThanRightNeighborIndices(int[] array) {
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив должен быть не null и содержать как минимум 1 элемент");
        }
        // Используем Stream для поиска индексов элементов, которые больше своего правого соседа
        return IntStream.range(0, array.length - 1) // Исключаем последний элемент
                .filter(i -> array[i] > array[i + 1]) // Элемент больше следующего
                .map(i -> i + 1) // Преобразуем индекс в порядковый номер
                .sorted() // Сортируем номера по возрастанию
                .toArray();
    }
    public static void main(String[] args) {
        ...
        // Поиск номеров элементов, больших правого соседа, и вывод результата
        try {
            int[] result = findGreaterThanRightNeighborIndices(array);
            System.out.print("Номера элементов, больших своего правого соседа: ");
            if (result.length == 0) {
